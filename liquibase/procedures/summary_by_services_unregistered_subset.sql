create or replace procedure summary_by_services_unregistered_subset(IN partition_date date DEFAULT (CURRENT_DATE - '1 day'::interval))
    language plpgsql
as
$$
DECLARE
			...
			FROM
				(
					...
					WHERE
						...
						ttb.previous_group_id IS NULL AND
						ttb.registered = TRUE AND 
						ttb.reprocessing_required = TRUE
				) AS sub
			WHERE
				date_of_journey = %L
			GROUP BY
...
alter procedure summary_by_services_unregistered_subset owner to abods_proxy_rw;
