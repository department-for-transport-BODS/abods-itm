...
'create table public.%I as 
	select 
		...
		day_of_week,
		stop_type,
		concat_ws(''|'', "operator",line_name,journey_code,date_of_journey) as group_id,
		a.atco_code,
		row_number() over(partition by "operator",line_name,journey_code,date_of_journey,stop_id,stop_index order by file_name ) as rk,
...
'Insert into public.%I (
	...
	select 
		...
		null as otp_state,
		extract(epoch from departure_time::time - lag(departure_time::time) over(partition by operator_ref, line_name, date_of_journey, stop_id, stop_index order by stop_id, stop_index, departure_time asc)) as expected_headway,
...
( select 
timetable_id,
extract(epoch from actual_departure_time - lag(actual_departure_time) 
over(partition by operator_noc, line_name, date_of_journey, stop_id, stop_index order by stop_id, stop_index asc,expected_departure_time asc)) as actual_headway
...
CREATE OR REPLACE PROCEDURE public.generate_retrospective_timetable(IN partition_date date)
 LANGUAGE plpgsql
AS $procedure$

declare 
longdatestring text:= to_char(partition_date, 'YYYY_MM_DD');
timetable_suffix text:= concat('_retro_', longdatestring);
tablename text:= 'Timetable';
...
execute format(
'create table public.%I as
with --get list of all potential timetable dataset ids (od2.dataset_type = 1)
...
ranked_revisions as (
 select pr.*,
 rank() over (partition by pr.dataset_id order by id desc) as id_rank
...
select distinct id as revision_id, dataset_id from highest_revisions
',
concat('potential_revisions', timetable_suffix),
partition_date,
partition_date
