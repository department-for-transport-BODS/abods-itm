    ...
    execute format(
            'SELECT * from aws_s3.query_export_to_s3(
                ...
                order by
                    ...
                    stop_index  asc
                '',
                aws_commons.create_s3_uri(concat(''abods-'', SPLIT_PART(aurora_db_instance_identifier(), ''-'', 2), ''-exporter-bucket''), ''historic/csv/timetable/YYYY=%s/MM=%s/%s.csv'',  ''eu-west-2''),
                options :=''format csv''
            )',
            datestring,
            yearstr,
...
begin
    ...
    execute format(
            ...
            aws_commons.create_s3_uri(concat(''abods-'', SPLIT_PART(aurora_db_instance_identifier(), ''-'', 2), ''-exporter-bucket''), ''historic/csv/siri/YYYY=%s/MM=%s/siri_vm_%s.csv'',  ''eu-west-2''),
        options :=''format csv''
        )',
            datestring,
            yearstr,
