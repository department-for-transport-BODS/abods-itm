-- CREATE UPDATE PERFORMANCE PROCEDURE --
		...
		FOREACH is_timing_point_var IN ARRAY is_timing_points LOOP
			...
			EXECUTE format('
				...
				SELECT 
					...
					CASE WHEN t.trend_total_count = 0 THEN 0 ELSE ((c.on_time_count::FLOAT / c.total_count)*100) - ((t.trend_on_time_count::FLOAT / t.trend_total_count)*100) END AS percentage_change
				...
				AND c.noc_and_line = t.noc_and_line
...
SELECT cron.schedule('update_performance_statistics_v4', 
	'10 3 * * *', $$CALL public.update_performance_statistics_v4()$$);
