...
def lambda_handler(event, context):
    ...
    try:
        ...
        for rec in event['Records']:
            ...
            cur.execute("Update public.batch set db_ingestion_status = 'Inprogress',db_ingestion_strt_prc_ts=%s where batch_id=%s ;",[start_time , batch_id])
        
            try:
                logging.info(f"Truncate the data for batch {batch_id} in staging table")
                cur.execute(f"delete from public.staging_avl_positions where batch_id = {batch_id} ;")
                ...
                logging.info(f"Data from staging tables to other tables positions,operators, linenames and journeys populated ")
                end_time = str(datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"))
                cur.execute("Update public.batch set db_ingestion_status = 'Success',db_ingestion_end_prc_ts=%s,s3_avl_gz_key=%s where batch_id=%s ;",[end_time ,key, batch_id])
                cur.close()    
        
            except Exception as e:
                end_time = str(datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"))
                cur.execute("Update public.batch set db_ingestion_status = 'Failed',db_ingestion_end_prc_ts=%s,s3_avl_gz_key=%s where batch_id=%s ;",[end_time ,key, batch_id])
