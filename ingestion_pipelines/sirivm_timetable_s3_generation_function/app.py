...
def read_historic_timetable(timetable_date):
    ...
    colnames = ["group_id", "stop_index", "stop_latitude", "stop_longitude", "expected_departure_time", "timetable_id", "date_of_journey"]
    try:
        df = wr.s3.read_csv(path=f"s3://{sirivm_bucket}/{timetable_filename}", names = colnames, header=None)
...
def recreate_timetable(timetable):
    ...
    for row in timetable:
        ...
        recreated_timetable[group_id][str(row["stop_index"])] = [[row["stop_latitude"], row["stop_longitude"]], row["expected_departure_time"], row["timetable_id"], row["date_of_journey"]]
        if count % 500000 == 0:
    ...
    try:
        for group_id, items in recreated_timetable.items():
            for stop, details in items.items():
                journey_time = datetime.strptime(f"{timetable_date} {details[1]}", '%Y-%m-%d %H:%M:%S')
                if journey_time > sliding_window_minus and journey_time < sliding_window_plus:
    ...
    if len(start_date_split) == 3 and len(end_date_split) == 3:
        while (timetable_end_datetime >= timetable_start_datetime):
            ...
            try:
                ...
                if 'Contents' in client.list_objects(Bucket=sirivm_bucket, Prefix=shredded_dir):
                    shredded_timetables_list = [obj['Key'] for obj in client.list_objects_v2(Bucket=sirivm_bucket, Prefix=shredded_dir, Delimiter = "/")['Contents'] if "/" != obj['Key'][-1]]
                    shredded_timetables_list.sort(key=lambda x: int(x[-10:-8] + x[-7:-5]))
                    ...
                    if number_of_shredded_timetables < 48:
                        ...
                        start_minute = 0 if shredded_timetables_list[-1][-7:-5] == "00" else 1
                logger.info(f"Starting shredding on {h_year}-{h_mon}-{h_day} at {start_hour}:{'00' if start_minute == 0 else '30'}")
                for h in range(start_hour, 24):
                    for i in range(start_minute, 2):
                        ...
                        query_time = datetime.strptime(f"{timetable_end_date} {hour_str}{minute_str}", '%Y-%m-%d %H%M')
                        file_name = shredded_dir + f"timetable_{h_year}{h_mon}{h_day}_{hour_str}_{minute_str}.json"
                        timetable_output = shred_timetable(recreated_timetable, query_time, timetable_end_date)
        ...
        logger.error(f"Input backfill date ({timetable_start_date}, {timetable_end_date}) is/are not in a valid format YYYY-MM-DD.")
    ...
    try:
        ...
        for shard_no in range(no_of_shards):
            ...
            resp = queue.send_message(
                ...
                MessageAttributes={ 'key': {'StringValue': 'timetable','DataType': 'String' }} )
            logging.info(f"Send message to  {otp_queue}{shard_no+1} so timetable is refreshed.")
    except Exception as e:
        logging.error(e)