...
def download_s3_dir(bucket_name: str, prefix_path: str, local_path: str):
    ...
    res = session.resource(service_name="s3", region_name=AWS_REGION)
    try:
        ...
        if s3_dir_exists(bucket_name, prefix_path):
            ...
            log.info(
                f"Objects under '{prefix_path}' downloaded successfully to '{local_path}'"
            )
        else:
            raise Exception(
                "Error downloading directory contents from S3. Provided path does not exist"
            )
...
def get_latest_object_dir(bucket_name: str, prefix_path: str):
    ...
    try:
        ...
        if not versioned_common_prefixes:
            log.error(
                f"No versioned objects have been found under the path {prefix_path}"
            )
            return None
        version_numbers = [
            tuple(int(v) for v in prefix.split("/")[-2].split("."))
            for prefix in versioned_common_prefixes
        ]
...
def s3_dir_exists(bucket_name: str, prefix_path: str) -> bool:
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "404":
            log.error(
                f"Path '{prefix_path}' does not exist under S3 bucket '{bucket_name}'"
            )
            return False
        else:
